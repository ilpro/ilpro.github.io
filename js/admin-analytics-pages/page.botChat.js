...
var pageMessages = {
  ...
  init: function (params) {
    ...
    // get user chat list
    lastChatUsers.init({userHash: user.info.hash, container: '.private-message-users', page: 'bot-chat'});
    searchUser.init({page: 'bot-chat'});

    // get user bots and get unread messages for bots every 10 sek
    if (user.info.userRole >= 50) {
      userBots.init({container: '.favour-users-block', infoContainer: '.private-messages-title'});

      getBotsMessagesCount(10000);
    }
    ...
    function getBotsMessagesCount(timer) {
      setTimeout(function (){
        userBots.getBotsUnreadMessagesCount();
        getBotsMessagesCount(timer);
      }, timer)
    }
  },
  ...
  loadChat: function () {
    ...
    if (page === 'messages' || page === 'bot-chat') {
      ...
      if (self.recipientId) {
        ...
        // check if bot selected
        if (userBots.selectedBot) {
          botId = userBots.selectedBot.userId;
        }
        ...
        setTimeout(function () {
          ...
          message.getPrivateMessages(userId, self.recipientId, botId);
  ...
  initHandlers: function () {
    ...
    $(document).off('click', '#private-chat-send-button').on('click', '#private-chat-send-button', function (e) {
      ...
      // if user choose bot, send message from bot
      if (!!userBots.selectedBot) {
        userObj = userBots.selectedBot;
      }

      // user cannot write to himself
      if (self.recipientId == userObj.userId) return;
    ...
    $(document).off('wheel', '.chat').on('wheel', '.chat', function (e) {
      ...
      if ($(this).scrollTop() <= 100 && self.firstMessageId !== false) {
        ...
        // check if bot selected
        var userId = userBots.selectedBot ? userBots.selectedBot.userId : user.info.userId;
        message.loadMoreMessages(userId, self.recipientId, self.firstMessageId);
  ...
  initSocketHandlers: function () {
    ...
    self.socket.off('privateChatMessages').on('privateChatMessages', function (data) {
      ...
      var botId = userBots.selectedBot ? userBots.selectedBot.userId : false;

      // check if user in same chat or increase message count and exit
      // if(data.recipientId == self.botId || data.recipientId == self.recipientId) {
  ...
  addPreviewHtmlMessage: function (userObj) {
    ...
    msgText = message.parseMessages(msgText);
  ...
  decreaseUnreadMessageCount: function (messagesArr) {
    ...
    // emit to get bots unread msg count
    if (urlHash.getState('page') === 'bot-chat' && userBots.selectedBot) {
      setTimeout(function () {
        userBots.getBotsUnreadMessagesCount();
      }, 500)
    }
  ...
  insertRecipientInfo: function (data) {
    ...
    var name = user.getName(data);

    // user name
    var userName = user.getName(data);
    ...
    $('.user-nickname', '.chat-user-title').empty().html('@'+name);
  ...
  update: function () {
    ...
    // get user chat list
    if (userBots.selectedBot) {
      lastChatUsers.getUsers(userBots.selectedBot.hash);
      // request: getPrivateChatMessages, response: privateChatMessages
      // message.getPrivateMessages(userBots.selectedBot.hash, self.recipientId);
    } else {
      lastChatUsers.getUsers();
    }
  },
