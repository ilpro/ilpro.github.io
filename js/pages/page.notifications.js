...
const pageNotifications = {
  svg: {
	...
	<path fill="#ccc" d="M14 4c-.2-2.4-1.8-4-3.8-4C9 0 7.6.7 7 2 6.3.6 5 0 3.8 0 1.8 0 .2 1.6 0 4v1.3c.4 1.3 1 2.4 2 3.3L7 13l5-4.4c1-1 1.6-2 2-3.3V4zm0 0"></path>\
      ...
      PostLike: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="23" viewBox="0 0 14 13" class="message-type">\
	<path fill="#ccc" d="M14 4c-.2-2.4-1.8-4-3.8-4C9 0 7.6.7 7 2 6.3.6 5 0 3.8 0 1.8 0 .2 1.6 0 4v1.3c.4 1.3 1 2.4 2 3.3L7 13l5-4.4c1-1 1.6-2 2-3.3V4zm0 0"></path>\
</svg>',
  ...
  init (socket) {
	...
	setTimeout(function(){
		notifications.updateNotificationCount(0);
		$('#notify-box .message-row.unreaded').removeClass("unreaded");
	}, 5000);
  },
  ...
  initSocketHandlers () {
    ...
    self.socket.emit('getNotificationsAll',JSON.stringify({hash: user.info.hash}));

	self.socket.off('notificationsAll').on('notificationsAll', function(data) {
      data = JSON.parse(data);
      self.insertAllNotifications(data);
    });

    self.socket.off('newNotification').on('newNotification', function(data) {
      data = JSON.parse(data);
	  self.insertOneNotification(data);
    });
  },
  ...
  initHandlers () {
    ...
    $('.make-all-readed').on('click', function() {
      notifications.updateNotificationCount(0);
      $('#notify-box .message-row.unreaded').removeClass("unreaded");
	  self.socket.emit('readAllNotifications',JSON.stringify({hash: user.info.hash}));
    });
  },
  ...
  notifyTemplate (data) {
	...
	switch(data.notificationType) {
      case "PhotoLike" :
      {
        html = '<a href="#page=profile&id={senderId}">{senderName}</a> вподобав/ла вашу <a href="#page=profile&id={receiverId}&imageId={imageId}">світлину</a>.';
		html = html.replaceMany({
			"senderId" : data.senderId, 
			"senderName" : '@'+data.userName,
			"receiverId" : data.receiverId, 
			"imageId" : data.notificationTypeId
		})
        break;
      }
      case "PostLike" :
      {
            html = '<a href="#page=profile&id={senderId}">{senderName}</a> вподобав/ла ваш <a href="#page=profile&id={receiverId}&ribbonId={ribbonId}">запис</a>.';
            html = html.replaceMany({
                "senderId" : data.senderId,
                "senderName" : '@'+data.userName,
                "receiverId" : data.receiverId,
                "ribbonId" : data.notificationTypeId
            })
        break;
      }
      case "Present" :
      {
        html = '<a href="#page=profile&id={senderId}">{senderName}</a>  відправив/ила тобі подарунок. <a href="#page=profile&id={receiverId}">Подивитися в профілі</a>.';
		html = html.replaceMany({
			"senderId" : data.senderId, 
			"senderName" : '@'+data.userName,
			"receiverId" : data.receiverId
      ...
      case "UserFavorite" :
      {
        html = '<a href="#page=profile&id={senderId}">{senderName}</a> на вас <a href="#page=subscribe">підписався</a>.';
		html = html.replaceMany({
			"senderId" : data.senderId, 
			"senderName" : '@'+data.userName
		})
	...
	var html = '<div class="message-row ' + ((!data.notificationRead) ? 'unreaded' : '') + '">\
	...
	' + self.svg[data.notificationType] + '\
</div>';

    return html;
  },

  insertOneNotification (data) {
    var self = this;
    
	$('#notify-box').prepend(self.notifyTemplate(data));
  },

  insertAllNotifications (notificationsArr) {
    const self = this;

    let htmlAll = '';

    notificationsArr.forEach(function (notificationObj, i, arr){
      htmlAll += self.notifyTemplate(notificationObj);
    });

    $('#notify-box').html(htmlAll);
  }
};