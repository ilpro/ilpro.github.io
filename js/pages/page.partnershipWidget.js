...
var pagePartnershipWidget = {
  ...
  initSocketHandlers() {
    ...
    // handle register
    self.socket.on('widgetDatingSitesList', function (data) {
      data = JSON.parse(data);
      self.insertSitesList(data);
      self.insertSavedUserInfo(data[0].userId, data[0].price);
      self.getStatistic(data[0].domainId, data[0].bannerId);
    });

    // handle register
    self.socket.on('widgetDatingStatistics', function (data) {
    ...
    self.socket.on('bindUserToWidgetDatingDomain', function (data) {
      if (data.success) {
    ...
    self.socket.on('bindPriceToWidgetDatingDomain', function (data) {
      if (data.success) {
        showNotify("success", lang.lProfileSave);
      }
      else {
  ...
  initEventHandlers () {
    ...
    $(".author").change(function () {
      ...
      var userId = $('.author option:selected').attr('data-userId');
      var price = $('.author option:selected').attr('data-price');
      self.getStatistic(domainId, bannerId);
      self.insertSavedUserInfo(userId, price);
    });
    ...
    $('.save-button').click(function () {
      ...
      if(price)  self.bindPriceToDomain(domainId, price);
  ...
  getStatistic (domainId, bannerId) {
    var self = this;
    self.socket.emit('getWidgetDatingStatistics', JSON.stringify({
      hash: user.info.hash,
      domainId: domainId,
      bannerId: bannerId
    }));
  ...
  /**  Bind user to domain  (for admin only) **/
  bindUserToDomain(domainId, userId) {
    var self = this;
    ...
    self.socket.emit('bindUserToWidgetDatingDomain', JSON.stringify(sendData));
  },

  /**  Bind price to domain  (for admin only) **/
  bindPriceToDomain(domainId, price) {
    var self = this;
    ...
    self.socket.emit('bindPriceToWidgetDatingDomain', JSON.stringify(sendData));
  },
  ...
  insertSavedUserInfo (userId, price) {
    if(+price){
      $('#price').val(+price);
    } else {
      $('#price').val('');
